...
export class LoginComponent implements OnInit {
  ...
  googleLogin () {
    let state = generateId(20)
    localStorage.setItem("state", state);
    this.windowRefService.nativeWindow.location.replace(`${oauthProviderUrl}?client_id=${this.clientId}&response_type=token&state=${state}&scope=email&redirect_uri=${this.redirectUri}`)
  }
}
